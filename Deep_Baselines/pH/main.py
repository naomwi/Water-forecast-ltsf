...
def run_lstm_ceemdan(horizon, data, device, site, verbose=True):
    ...
    try:
        ...
        CACHE_DIR = PROJECT_DIR / "Deep_Baselines" / "pH" / "cache"
    except ImportError:
    ...
    result = get_or_create_imfs(data, CACHE_DIR, prefix="ph", n_imfs=12)
...
def _save_results(model_type, horizon, actuals, preds, metrics, site):
    ...
    series_path = RESULTS_DIR / "series" / f"series_{model_type}_site{site}_P{horizon}_pH.csv"
    series_df.to_csv(series_path, index=False)

    # Plot comparison
    (RESULTS_DIR / "plots").mkdir(parents=True, exist_ok=True)
    plot_path = RESULTS_DIR / "plots" / f"series_{model_type}_site{site}_P{horizon}_pH.png"
    plot_prediction(
        actual=actuals,
        predicted=preds,
        title=f"Comparison: series_{model_type}_site{site}_P{horizon}_pH",
        save_path=plot_path
    )
