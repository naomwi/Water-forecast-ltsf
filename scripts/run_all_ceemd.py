"""
Run ALL CEEMD Baseline experiments: 5 sites × 6 horizons × 2 targets = 60 runs.
CEEMD IMF cache is per-site, so decomposition runs once per site per target.
"""
import subprocess
import sys
import time
...
print(f"{'='*60}")
print(f"CEEMD BASELINES - FULL RUN: {total} experiments")
print(f"Sites: {SITES}")
...
for target in TARGETS:
    for site in SITES:
        for h in HORIZONS:
            ...
            cmd = [
                sys.executable,
                f"CEEMD_Baselines/{target}/main.py",
                "--horizon", str(h),
                "--site", str(site),
            ]
