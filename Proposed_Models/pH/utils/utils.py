...
def load_and_preprocess_data(file_path, target_col, train_ratio=0.7, site=1463500, cache_dir="data/cache"):
    ...
    high_path = os.path.join(cache_dir, f"imfs_{site}_raw_high_{target_col.lower()}.npy") # Đổi tên file cache
    low_path = os.path.join(cache_dir, f"imfs_{site}_raw_low_{target_col.lower()}.npy")
    ...
    if os.path.exists(high_path) and os.path.exists(low_path):
        ...
        if temp_high.shape[0] != n_total: 
            print("-> Old cache length mismatch. Deleting cache...")
            shutil.rmtree(cache_dir)
            os.makedirs(cache_dir, exist_ok=True)
            force_run = True

    if os.path.exists(high_path) and os.path.exists(low_path) and not force_run:
        print(f"-> Loading cached Raw IMFs ({FIXED_NUM_IMFS} components)...")
        high_imfs_raw = np.load(high_path)
        low_imfs_raw = np.load(low_path)
    else:
        print(f"-> Running CEEMDAN on Raw Data (Parallel)...")
        ...
        print("-> Processed Raw decomposition and saved to Cache.")
...
def plot_results(preds, actuals, pred_len, save_path):
    ...
    plt.subplot(2, 1, 1)
    plt.plot(actuals[:, -1], label='Actual', color='blue', alpha=0.6, linewidth=1)
    plt.plot(preds[:, -1], label='Prediction',linestyle='--', color='#d62728', alpha=0.8, linewidth=1)
    plt.title(f'Test Set Performance (Last-Step Forecast) - Horizon {pred_len}')
    plt.legend()
